...
describe("Vault contract", function () {
  ...
  describe("Exchange Ethers", function () {
    it("Should send ethers", async function () {
      const previousBalance = await hardhatTokenContract.balanceOf(
        owner.address
      );
      const tokensToExchange = 50;
      await hardhatTokenContract
        .connect(addr2)
        .approve(owner.address, tokensToExchange);
      ...
      await mockTokenContract.mock.transferFrom
        .withArgs(addr2.address, owner.address, tokensToExchange)
        .returns(true);
  ...
  describe("Administrators", function () {
    it("Should add Administrator", async function () {
      expect(await hardhatVault.addAdmin(addr1.address)).to.equals(true);
    });
    it("Should revert add Administrator if caller not admin", async function () {
      await expect(
        hardhatVault.connect(addr2.address).addAdmin(addr1.address)
      ).to.be.revertedWith("Function accessible only by an admin");
    });
    ...
    it("Should revert remove Administrator if caller not admin", async function () {
      await expect(
        hardhatVault.connect(addr2.address).removeAdmin(addr1.address)
      ).to.be.revertedWith("Function accessible only by an admin");
    });
  });
});
